...
# Function to print each student's grades and average
def get_name_grades_with_avg(journal):
    """
    Prints each student's grades and their average.
    Also prints the student with the highest average and most consistent performance.
    """
    student_avg_grade = {}
...
def students_with_grade_below(journal, threshold=70):
    """
    Returns a list of students who have at least one grade below the given threshold.
    """
    students = []
    for name, grades in journal.items():
...
# Function to count total grades entered for the whole class
def total_grades_entered(journal):
    """
    Returns the total number of grades entered across all students.
    """
